...
export default function SupplierFormPage() {
  ...
  const [form, setForm] = useState({
    ...
    address: '', city: '', state: '', zip: '', notes: '', is_active: true
  })
  ...
  async function loadRow() {
    ...
    if (error) { toast.error(error.message); navigate('/suppliers'); return }
    setForm({
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (isEdit) {
        ...
        await logActivity('updated', 'supplier', id, payload.company_name)
      } else {
        payload.created_by = profile?.id
        const { data, error } = await supabase.from('suppliers').insert(payload).select().single()
        if (error) throw error
        await logActivity('created', 'supplier', data.id, payload.company_name)
      }
    ...
    <div className="max-w-3xl">
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          ...
          <CardBody className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <FormField label="State"><Input value={form.state} onChange={(e) => u('state', e.target.value)} maxLength={2} /></FormField>
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button type="submit" loading={saving}>{isEdit ? 'Update' : 'Create'} Supplier</Button>
        </div>
      </form>
    </div>
  )
